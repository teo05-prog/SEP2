...
class AuthenticationServiceImplTest {
  ...
  void register_successful() throws Exception {
    ...
    // Use ArgumentCaptor to verify the TravellerRequest passed to userService
    ArgumentCaptor<TravellerRequest> travellerRequestCaptor = ArgumentCaptor.forClass(TravellerRequest.class);

    String result = authenticationService.register(request);

    assertEquals("Success", result);
    verify(mockUserDAO, times(1)).readByEmail("new@example.com");
    verify(mockUserService, times(1)).createTraveller(travellerRequestCaptor.capture());
    TravellerRequest capturedRequest = travellerRequestCaptor.getValue();
    ...
    assertEquals(birthDate, capturedRequest.getBirthDate());
  }

  @Test
  void register_nameCannotBeEmpty() throws Exception {
    MyDate birthDate = new MyDate(1, 1, 2000, 0, 0);
    ...
    verify(mockUserService, never()).createTraveller(any(TravellerRequest.class));
  }

  @Test
  void register_nameCannotBeNull() throws Exception {
    MyDate birthDate = new MyDate(1, 1, 2000, 0, 0);
  ...
  @Test
  void register_invalidEmail_format() throws Exception {
    MyDate birthDate = new MyDate(1, 1, 2000, 0, 0);
  ...
  @Test
  void register_invalidEmail_empty() throws Exception {
    MyDate birthDate = new MyDate(1, 1, 2000, 0, 0);
  ...
  @Test
  void register_invalidEmail_null() throws Exception {
    MyDate birthDate = new MyDate(1, 1, 2000, 0, 0);
  ...
  void register_passwordTooShort() throws Exception {
    ...
    assertEquals("Password must be at least 8 characters long", result); // Assuming 8 based on your code, adjust if different
    verify(mockUserDAO, never()).readByEmail(anyString());
    verify(mockUserService, never()).createTraveller(any(TravellerRequest.class));
  }

  @Test
  void register_passwordNull() throws Exception {
    MyDate birthDate = new MyDate(1, 1, 2000, 0, 0);
  ...
  @Test
  void register_userServiceThrowsException() throws Exception {
    MyDate birthDate = new MyDate(1, 1, 2000, 0, 0);
    RegisterRequest request = new RegisterRequest("New User", "new@example.com", "password123", birthDate);
    when(mockUserDAO.readByEmail("new@example.com")).thenReturn(null);
    doThrow(new RuntimeException("DB error")).when(mockUserService).createTraveller(any(TravellerRequest.class));
    ...
    verify(mockUserDAO, times(1)).readByEmail("new@example.com");
    verify(mockUserService, times(1)).createTraveller(any(TravellerRequest.class));
  }
}